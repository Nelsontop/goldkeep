...
- **分类筛选与排序** — 按全部/首饰/金条筛选资产列表，首饰支持细分（手镯、项链、戒指、耳环、吊坠、手链）。支持按持有收益、买入总价、买入日期、买入克价排序，默认按持有收益倒序
- **持仓概览** — 首页汇总总克重、总买入金额、当前持仓价值，每项资产展示浮动盈亏
- **金价走势** — 每日金价折线图，每小时自动从公开 API 获取最新金价
- **客户端缓存** — 资产列表和金价数据缓存在内存中，页面切换即时展示，写操作自动更新缓存。图片通过 HTTP 缓存头实现 30 天浏览器缓存
- **照片上传** — 每个资产支持上传实物照片，详情页支持照片灯箱放大查看
- **暗色主题** — Binance 风格暗色 UI，深色画布 + 金色点缀，lucide-react 线框图标
...
前端运行在 `http://localhost:19090`，后端 API 运行在 `http://localhost:19091`。
